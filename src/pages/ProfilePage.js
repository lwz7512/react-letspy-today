...
const ProfilePage = () => {
  ...
  const loginSuccessHandler = (credentialResponse) => {
    ...
    const timestamp = month > 9 ? month : '0' + month + '/' + day + '/' + today.getFullYear()
    // save first
    ...
    <div className='profile button-social social-bg mt-8 sm:mt-4'>
      ...
      {!isLoggedIn && (
        <div className='social-signin card p-5 my-8 border-solid border-500 '>
          ...
          <p className='text-lg'>
            {' '}
            <b>Note:</b> this is not necessary, the benefit to do this is you
            could get a <b>`cheatsheet`</b> bonus after you sucessfully
            completed one season projects(at least 9), also I could know whom
            this project have brought achievement to!
          </p>
        </div>
      )}
